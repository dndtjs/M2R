...
for n in range(num_sims):
    ...
    if x:
        num_players += 1
        true_elos = list(true_elos)
        true_elos.append(np.random.normal(1500, 300))
        elos.append(1500)
        true_ranking = [[i, true_elos[i]] for i in range(num_players)]
        true_ranking.sort(key=lambda x: x[1], reverse=True)
        dic_true = {true_ranking[i][0]: (i + 1, true_ranking[i][1]) for i in range(num_players)}
        dic_count[num_players - 1] = 0
        dic_K[num_players - 1] = 0
    ...
    samples = random.sample(matches, len(matches) // 2)
    for i, j in samples:
        for k in (i, j):
            if dic_K[k] != 10:
                if dic_count[k] >= 30:
    ...
    l = [abs(dic_true[i][1] - dic_est[i][1]) for i in range(num_players)]
